...
class ContainerFactory(object):
    ...
    def create(self, scac, **kwargs):
        ...
        if not builder:
            raise ValueError(f"No builder registered for SCAC: {scac}")

        # Map `cn` to `container` for builder compatibility
        kwargs['container'] = kwargs.pop('cn', None)
        if not kwargs['container']:
            raise ValueError("Missing required 'container' argument.")
